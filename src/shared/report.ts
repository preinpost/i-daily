...
import { fmtMeta, guessSpace, type TaskRow } from "./model.ts";
...
export function weekWindow(ref: Date = new Date()): { from: string; to: string } {
...
export type RawEntry = { side: string; space: string; key: string; desc: string; progress: number | null; due: string; subs: string[] };
export type RawDay = { date: string; entries: RawEntry[] };
export type WeeklyDigest = { from: string; to: string; owner: string; spaces: DigestSpace[]; count: number; raw: RawDay[] };
...
// 스페이스는 row.space 우선, 비어있으면 guessSpace(티켓 prefix→스페이스)로 보완.
// 진척=최댓값, 마감=최신, 날짜/노트=합집합.
type ResolvedTask = DigestTask & { space: string };
export function buildWeeklyDigest(rows: TaskRow[], owner = "", from = "", to = ""): WeeklyDigest {
  ...
  for (const r of rows) {
    ...
    const rowSpace = (r.space || "").trim() || guessSpace(key);
    ...
    if (!cur) {
      ...
      if (typeof r.progress === "number") cur.progress = cur.progress == null ? r.progress : Math.max(cur.progress, r.progress);
  ...
  for (const dk of order) {
    ...
    t.dates.sort();
    if (!bySpace.has(label)) { bySpace.set(label, []); spaceOrder.push(label); }
    bySpace.get(label)!.push({ key: t.key, desc: t.desc, progress: t.progress, due: t.due, dates: t.dates, notes: t.notes, subs: t.subs });
  }
  const spaces: DigestSpace[] = spaceOrder.map((label) => ({ label, tasks: bySpace.get(label)! }));
...
function buildRaw(rows: TaskRow[]): RawDay[] {
  ...
  for (const r of rows) {
    ...
    byDate.get(date)!.push({
      side: r.side, space: (r.space || "").trim(), key, desc,
      progress: typeof r.progress === "number" ? r.progress : null,
      due: r.due || "", subs: (r.subs || []).slice(),
    });
  }
...
export function renderDigestText(d: WeeklyDigest): string {
  const L: string[] = [];
  if (!d.spaces.length) { L.push("(해당 기간 항목 없음)"); return L.join("\n"); }
  d.spaces.forEach((sp, i) => {
    if (i > 0) L.push("");   // 스페이스 블록 사이에만 빈 줄
    L.push(`[${sp.label}]`);
    for (const t of sp.tasks) {
      const head = t.key ? `[${t.key}]${t.desc ? " " + t.desc : ""}` : t.desc || "(내용)";
      L.push(`ㅇ${head}${fmtMeta(t.progress, t.due)}`);
      // 하위 불릿: 실제 subs 우선, 없으면 요일별 notes 파편으로 대체.
      const subs = t.subs.length ? t.subs : t.notes.filter((n) => n && n !== t.desc);
...
export function buildAgentPrompt(d: WeeklyDigest, customPrompt?: string): string {
  const tpl = (customPrompt && customPrompt.trim()) ? customPrompt : DEFAULT_REPORT_PROMPT;
...
function digestForPrompt(d: WeeklyDigest) {
  return {
    from: d.from, to: d.to, owner: d.owner,
    spaces: d.spaces.map((sp) => ({
      ...
      tasks: sp.tasks.map((t) => ({
        ...
        notes: t.subs.length ? undefined : (t.notes.length ? t.notes : [t.desc].filter(Boolean)),
      })),
    })),
  };
}
