...
function createWindow(): void {
  ...
  win.on("close", (e) => {
    ...
    if (choice === 2) { quitting = false; return; }   // 취소 → 유지
    if (choice === 1) { proceedClose(); return; }      // 저장 안 함 → 종료
...
app.whenReady().then(() => {
  ...
  const dbPath = process.env.DB_PATH ?? join(app.getPath("userData"), "i-daily.db");
  db = openDb(dbPath);

  // 프런트 api(method,path,body) → route → {status, body}. 전송만 HTTP→IPC로 교체.
  ipcMain.handle("api", (_e, req: { method: string; path: string; body: unknown }) =>
