...
//
// 토큰은 userData/jira-auth.json 에 safeStorage(OS 키체인)로 암호화 저장. access token(1h)
...
type Tokens = {
  accessToken: string;
  refreshToken: string;
  expiresAt: number; // epoch ms (만료 60초 전으로 저장)
  cloudId: string;
  siteUrl: string;
  siteName: string;
};
...
let _pending: { state: string; resolve: (code: string) => void; reject: (e: Error) => void; timer: ReturnType<typeof setTimeout> } | null = null;

export function setupJira(db: Database.Database, user: string): void {
  _db = db;
  _user = user;
  _authFile = join(app.getPath("userData"), "jira-auth.json");

  ipcMain.handle("jira:status", () => statusPayload());
  ipcMain.handle("jira:connect", () => connect());
  ipcMain.handle("jira:logout", () => { clearTokens(); return statusPayload(); });
  ipcMain.handle("jira:tickets", () => tickets());
}

// ───────────────────────── 토큰 저장(safeStorage 암호화) ─────────────────────────
function saveTokens(t: Tokens): void {
  const json = JSON.stringify(t);
  const buf = safeStorage.isEncryptionAvailable()
    ? safeStorage.encryptString(json)
    : Buffer.from(json, "utf8");
  writeFileSync(_authFile, buf);
}
function loadTokens(): Tokens | null {
  if (!existsSync(_authFile)) return null;
  try {
    const buf = readFileSync(_authFile);
    const json = safeStorage.isEncryptionAvailable()
      ? safeStorage.decryptString(buf)
      : buf.toString("utf8");
    return JSON.parse(json) as Tokens;
  } catch {
    return null; // 복호화 실패(키체인 변경 등) → 재연결 유도
  }
}
function clearTokens(): void {
  try { if (existsSync(_authFile)) unlinkSync(_authFile); } catch { /* noop */ }
}
...
async function connect(): Promise<any> {
  ...
  if (!clientId || !clientSecret) {
    return { ok: false, error: "먼저 ⚙️ 설정에 Jira client id / secret 을 입력하세요." };
  }
  try {
...
function ensureServer(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    const server = createServer((req, res) => {
      ...
      res.end(`<!doctype html><meta charset="utf-8"><body style="font-family:system-ui,sans-serif;padding:48px;text-align:center;color:#222">`
      ...
      if (gotState !== cur.state) return cur.reject(new Error("state 불일치(재시도)."));
      cur.resolve(gotCode);
    });
    server.on("error", reject); // EADDRINUSE 등 → connect()에서 표면화
    server.listen(CALLBACK_PORT, "127.0.0.1", () => { _server = server; resolve(); });
  });
}
...
async function runOAuth(clientId: string, state: string): Promise<string> {
  await ensureServer();
  if (_pending) { // 이전 시도 취소
    clearTimeout(_pending.timer);
    const prev = _pending; _pending = null;
    prev.reject(new Error("replaced")); // 새 인증 시도로 대체
  }
  return new Promise<string>((resolve, reject) => {
    const timer = setTimeout(() => {
      if (_pending && _pending.timer === timer) { _pending = null; reject(new Error("인증 시간 초과(3분). 다시 시도하세요.")); }
    }, 3 * 60 * 1000);
    ...
    void shell.openExternal(`${AUTHORIZE_URL}?${q}`).catch((e) => {
      if (_pending && _pending.timer === timer) { clearTimeout(timer); _pending = null; reject(new Error("브라우저 열기 실패: " + msg(e))); }
    });
  });
}

async function exchangeCode(clientId: string, clientSecret: string, code: string): Promise<{ access_token: string; refresh_token: string; expires_in: number }> {
...
async function resolveSite(accessToken: string): Promise<{ cloudId: string; siteUrl: string; siteName: string }> {
  const r = await fetch(RESOURCES_URL, { headers: { Authorization: `Bearer ${accessToken}`, Accept: "application/json" } });
  if (!r.ok) throw new Error(`사이트 조회 실패 (${r.status})`);
  const list = (await r.json()) as Array<{ id: string; url: string; name: string }>;
  if (!list.length) throw new Error("접근 가능한 Jira 사이트가 없습니다.");
  const cfg = readConfig(_db, _user);
  const wantHost = (cfg.jiraBase || "").replace(/^https?:\/\//, "").replace(/\/.*$/, "").toLowerCase();
  const pick = (wantHost && list.find((s) => (s.url || "").toLowerCase().includes(wantHost))) || list[0];
...
async function getValidToken(): Promise<Tokens> {
  const t = loadTokens();
  if (!t) throw new Error("Jira 에 연결되어 있지 않습니다. ⚙️ 설정에서 연결하세요.");
  ...
  if (!r.ok) { clearTokens(); throw new Error(`세션 갱신 실패 (${r.status}) — 다시 연결하세요.`); }
...
async function tickets(): Promise<any> {
  let t: Tokens;
  try { t = await getValidToken(); } catch (e) { return { ok: false, error: msg(e) }; }
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const r = await fetch(`${base}?${q}`, { headers: { Authorization: `Bearer ${t.accessToken}`, Accept: "application/json" } });
