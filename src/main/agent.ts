...
import { buildWeeklyDigest, renderDigestText, buildAgentPrompt, weekWindow, DEFAULT_REPORT_PROMPT } from "../shared/report.ts";
import { readConfig } from "../shared/store.ts";
import { setConfig } from "../shared/model.ts";

// 각 CLI의 비대화(non-interactive) 실행 스펙. 프롬프트는 마지막 인자로 전달(세 CLI의
// 문서화된 `-p "…"` / `exec "…"` 형식). 실제 플래그가 다르면 args만 조정하면 된다.
type AgentSpec = { id: string; label: string; bin: string; args: string[]; versionArgs: string[] };
const AGENTS: AgentSpec[] = [
  { id: "claude", label: "Claude Code", bin: "claude", args: ["-p"], versionArgs: ["--version"] },
  { id: "codex", label: "Codex", bin: "codex", args: ["exec"], versionArgs: ["--version"] },
  { id: "pi", label: "pi", bin: "pi", args: ["-p"], versionArgs: ["--version"] },
];
...
export function setupAgent(db: Database.Database, user: string): void {
  ...
  ipcMain.handle("agent:generate", (_e, opts: GenerateOpts) => generate(opts || {}));
...
// ───────────────────────── 스캔 ─────────────────────────
type Scanned = { id: string; label: string; bin: string; path: string; version: string };
...
function getVersion(path: string, args: string[]): Promise<string> {
  return new Promise((resolve) => {
    execFile(path, args, { env: spawnEnv(), timeout: 5000 }, (err, stdout, stderr) => {
...
async function generate(opts: GenerateOpts): Promise<any> {
  ...
  if (!opts.useAgent || digest.count === 0) {
    return { ok: true, from, to, count: digest.count, text: deterministic, deterministic, usedAgent: null };
  }
  ...
  try {
    ...
    return { ok: true, from, to, count: digest.count, text, deterministic, usedAgent: spec.label };
  } catch (e) {
    return { ok: true, from, to, count: digest.count, text: deterministic, deterministic, usedAgent: null,
...
function runAgent(path: string, args: string[]): Promise<string> {
  return new Promise((resolve, reject) => {
    const child = spawn(path, args, { env: spawnEnv(), stdio: ["ignore", "pipe", "pipe"] });
    let out = "", err = "";
    const timer = setTimeout(() => { child.kill(); reject(new Error("타임아웃(90초)")); }, 90_000);
    ...
    child.on("close", (code) => {
      ...
      else reject(new Error(`exit ${code}: ${(err || out).trim().slice(0, 200)}`));
    });
  });
}
