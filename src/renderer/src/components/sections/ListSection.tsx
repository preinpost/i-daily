...
import { itemId, listHasContent, syncScrumFromDailyChange } from "../../lib/model";
...
export function ListSection({ sec, curDate }: { sec: ListSec; curDate: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      if (!r.ok || !r.json || !Array.isArray(r.json.items)) return setPrevMap({});
  ...
  async function importYesterday() {
    const r = await api<any>("GET", `/api/day/${curDate}/prev-daily`);
    if (!r.ok || !r.json || !r.json.count) return toast("가져올 어제 일일 기록이 없어요");
    ...
    const hasId = (id: string) => (sec.items || []).some((it) => itemId(it) === id);
    let added = 0;
  ...
  return (
    <div className="mb-[22px]">
      ...
      <div className="grid gap-1.5">
        ...
        {sec.items.map((it, i) => (
          <ListItemRow key={i} sec={sec} it={it} index={i} scrum={doc.scrum} prev={prevMap[itemId(it)]} />
        ))}
      </div>
      ...
      <button
        ...
        onClick={() => {
          sec.items.push({ done: false, key: "", desc: "", progress: "", due: "", subs: [] });
          commit();
        }}
      >
        + 항목
...
function ListItemRow({
  ...
  return (
    <div
      className={
        "drag-row flex flex-wrap items-center gap-2 rounded-lg bg-panel-2 p-2 " + (over ? "dragover" : "")
      }
      {...props}
    >
