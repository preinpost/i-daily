...
export function ScrumBlock({ which, curDate }: { which: Which; curDate: string }) {
  ...
  // 빈 영역으로 드롭 → prefix 로 스페이스 추론해 등록
  const { over, props } = useCopyZone((item) => {
    const r = copyListItemToScrum(scrum, config, which, item, null);
    toast(r.msg);
    if (r.ok) commit();
  });

  async function importPrev() {
    const has = (scrum.prev.spaces || []).some((sp) => (sp.tasks || []).some((t) => t.key || t.desc));
    if (has && !confirm("전일 진행 업무를 직전 근무일의 '일일 진행 업무'로 덮어쓸까요?")) return;
  ...
  function syncToday() {
    const items = todayDailyItems(doc).filter((it) => (it.key || "").trim() || (it.desc || "").trim());
    ...
    const dup = (it: ListItem) => {
      ...
      return (scrum.today.spaces || []).some((sp) =>
        (sp.tasks || []).some((t) =>
          key ? (t.key || "").trim().toUpperCase() === key : (t.desc || "").trim() === desc,
        ),
      );
    };
    let added = 0;
    items.forEach((it) => {
      if (dup(it)) return;
      const spLabel = guessSpace(config, it.key);
      let sp = (scrum.today.spaces || []).find((s) => (s.label || "") === spLabel);
      if (!sp) {
        sp = { label: spLabel, tasks: [] };
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-line bg-panel-2">
      <div className="flex items-center gap-2 border-b border-line bg-panel px-3.5 py-[11px]">
        ...
        <span className="rounded-full bg-chip px-2 py-0.5 text-[11px] text-ink-2">{tag}</span>
        ...
        <button
          ...
          className="btn btn-tiny btn-ghost"
          onClick={() => {
            block.spaces.push({ label: "", tasks: [{ key: "", desc: "", progress: "", due: "", subs: [] }] });
            commit();
          }}
        >
          + 스페이스
      ...
      <div className="px-3.5 pb-3.5 pt-3">
        <div className={"rounded-[10px] " + (over ? "dragover-copy" : "")} {...props}>
          {!block.spaces.length && (
            <div className="whitespace-pre-line p-3.5 text-center text-[13px] text-ink-2">
              {"스페이스를 추가한 뒤 일일 진행 업무에서 항목을 선택하세요\n"}
              <span className="mt-1.5 block text-xs opacity-85">또는 일일 진행 업무를 여기로 드래그</span>
            </div>
          )}
          {block.spaces.map((sp, si) => (
            <ScrumSpace key={si} space={sp} block={block} which={which} index={si} />
          ))}
        </div>
        ...
        <div className="mt-1.5">
          <label className="mb-[3px] block text-xs text-ink-2">협업 및 기타</label>
          <input
            placeholder="없음"
            value={block.collab || ""}
            onChange={(e) => {
