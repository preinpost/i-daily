...
export function ScrumSpace({
	...
	const { over, props } = useCopyZone((item) => {
		const r = copyListItemToScrum(doc.scrum, which, item, sp);
		toast(r.msg);
		if (r.ok) commit();
	});
	...
	return (
		<div
			className={
				"mb-2.5 rounded-[10px] border border-dashed border-line bg-panel p-2.5 " +
				(over ? "dragover-copy" : "")
			}
			{...props}
		>
			<div className="mb-2 flex items-center gap-2">
				...
				<button
					...
					onClick={() => {
						if (!dailyOptions(doc).length)
							return toast("일일 진행 업무에 먼저 항목을 추가하세요");
						sp.tasks.push({
							key: "",
							desc: "",
							progress: "",
