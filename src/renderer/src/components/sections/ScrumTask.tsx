...
export function ScrumTask({
  ...
  return (
    <div
      ...
      {...props}
    >
      <DragHandle arr={sp.tasks} index={index} className="!absolute left-0.5 top-2 !px-0" />
      ...
      <label className="flex min-w-0 flex-col gap-[3px] text-xs text-ink-2">
        ...
        <span className="flex min-w-0 items-center gap-1.5">
          <span
            ...
            title={order != null ? `일일 진행 업무 ${order}번` : "일일 진행 업무에 연결되지 않음"}
          >
            {order ?? "–"}
          </span>
          <select
            ...
            onChange={(e) => {
              ...
              if (!id) {
                ...
                if (src) {
                  ...
                  t.progress = typeof src.progress === "number" ? src.progress : "";
                  t.due = src.due || "";
                  if (t.key && !sp.label) {
                    const g = guessSpace(config, t.key);
                    if (g) sp.label = g;
                  }
                }
              }
          ...
          >
            ...
            {daily.map((it) => {
              ...
              return (
                <option key={id} value={id} disabled={used.has(id) && id !== curId}>
                  {dailyItemLabel(it)}
                </option>
              );
            })}
            {orphan && <option value={curId}>{dailyItemLabel(t) + " (일일에 없음)"}</option>}
          </select>
        </span>
      </label>
      ...
      {warn && <div className="col-span-full text-xs text-warn">⚠ 진척률·마감일을 채워 주세요 (규정 2)</div>}
      ...
      {t.subs.some((s) => (s || "").trim()) && (
        <div className="col-span-full">
          {t.subs
            .filter((s) => (s || "").trim())
            .map((s, i) => (
              <div key={i} className="sub-bullet mt-1 ml-2 flex items-center gap-1.5">
