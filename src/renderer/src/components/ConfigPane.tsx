...
import { rulesToText, textToRules } from "../lib/model";
import type { Config } from "../types";

type JiraStatus = { connected?: boolean; configured?: boolean; site?: string; siteUrl?: string; redirectUri?: string } | null;

const fieldCls =
  "rounded-[9px] bg-panel-2 px-3 py-[9px] text-[13px]";
...
export function ConfigPane({
  ...
  const [rules, setRules] = useState(rulesToText(config.spaceRules));
  const [clientId, setClientId] = useState(config.jiraClientId || "");
  const [clientSecret, setClientSecret] = useState(config.jiraClientSecret || "");
  const [reportAgent, setReportAgent] = useState(config.reportAgent || "");
  const [agents, setAgents] = useState<{ id: string; label: string; version: string; path: string }[] | null>(null);
  ...
  useEffect(() => {
    ...
    setRules(rulesToText(config.spaceRules));
  ...
  async function save() {
    ...
    if (r.ok && r.json && r.json.config) {
      ...
      toast(r.json.configured ? "설정 저장됨" : "저장됨 — Jira 주소·이름은 필수예요");
      refreshJira();
    } else toast("설정 저장 실패");
  }
  ...
  async function connect() {
    ...
    toast(r && r.ok ? "Jira 연결됨" : "Jira 연결 실패: " + ((r && r.error) || "알 수 없음"));
    refreshJira();
  }
      ...
      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-4 px-5 pb-12 pt-7">
        ...
        {firstRun && !hideFirstRun && (
          <p className="tint-accent m-0 rounded-[10px] px-3.5 py-2.5 text-[13px] text-ink">
            처음 실행이에요. 아래 값을 채우면 업무일지가 활성화됩니다. (Jira 주소·이름 필수)
          </p>
        )}

        <label className="flex flex-col gap-1.5">
          <span className="text-[13px] font-bold text-ink">이름 (owner)</span>
          <input className={fieldCls} placeholder="홍길동" value={owner} onChange={(e) => setOwner(e.target.value)} />
        </label>

        <label className="flex flex-col gap-1.5">
          <span className="text-[13px] font-bold text-ink">Jira 호스트 URL</span>
          <input
            className={fieldCls}
          ...
          <small className="text-xs text-ink-2">
            호스트만 넣으면 됩니다 — <code className="rounded-[5px] bg-panel px-[5px] py-px font-mono">/browse/티켓</code> 은
            자동으로 붙습니다.
          </small>
        </label>
        ...
        <label className="flex flex-col gap-1.5">
          ...
          />
          <small className="text-xs text-ink-2">스페이스 입력 자동완성 목록.</small>
        </label>

        <label className="flex flex-col gap-1.5">
          <span className="text-[13px] font-bold text-ink">스페이스 규칙 (티켓 prefix → 스페이스)</span>
          <textarea
          ...
          />
          <small className="text-xs text-ink-2">
            형식 <code className="rounded-[5px] bg-panel px-[5px] py-px font-mono">PREFIX=스페이스</code>. 티켓 키
            앞글자로 스페이스를 자동 추천합니다.
          </small>
        </label>
        ...
        <p className="tint-accent m-0 rounded-[10px] px-3.5 py-2.5 text-xs text-ink">
          <a href="https://developer.atlassian.com/console/myapps/" target="_blank" rel="noopener">
            developer.atlassian.com
          </a>{" "}
          → <b>OAuth 2.0 (3LO)</b> 앱 생성 → Permissions 에 <b>Jira API</b>(read:jira-work, read:jira-user) 추가 →
          Authorization 의 <b>Callback URL</b> 에 아래 값을 그대로 등록 → Settings 의 client id/secret 을 아래에 붙여넣으세요.
          <br />
          Callback URL:{" "}
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[13px] font-bold text-ink">Jira Client Secret</span>
          <input
            className={fieldCls}
            type="password"
            placeholder="••••••••"
          ...
          />
          <small className="text-xs text-ink-2">
            secret 은 로컬에만 저장되며, 발급된 토큰은 OS 키체인으로 암호화됩니다.
          </small>
        </label>
        ...
        <p className="tint-accent m-0 rounded-[10px] px-3.5 py-2.5 text-xs text-ink">
          주간보고의 <b>티켓키·진척%·마감</b>은 앱이 확정합니다. 에이전트는 <b>서술만</b> 자연스럽게 다듬어요.
          선택하지 않으면 결정적 집계 텍스트만 생성합니다(에이전트 불필요).
        </p>
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-3">
            <button type="button" className="btn btn-ghost" onClick={scanAgents} disabled={scanning}>
              {scanning ? "스캔 중…" : "🔍 에이전트 스캔"}
            </button>
            <span className="text-[13px] text-ink-2">
              {agents == null ? "—" : agents.length ? `${agents.length}개 발견` : "발견된 에이전트 없음"}
            </span>
          </div>
          <label className="flex cursor-pointer items-center gap-2 text-[13px] text-ink">
            <input type="radio" name="reportAgent" checked={!reportAgent} onChange={() => setReportAgent("")} />
            사용 안 함 (결정적 집계만)
          </label>
          {(agents || []).map((a) => (
            <label key={a.id} className="flex cursor-pointer items-center gap-2 text-[13px] text-ink">
              <input
                type="radio"
                name="reportAgent"
              ...
              />
              {a.label}
              {a.version && <span className="text-xs text-ink-2">· {a.version}</span>}
            </label>
          ))}
        </div>
        ...
        <div className="mt-1 flex items-center gap-3">
          ...
          <span className="text-[13px] text-ink-2">{savedAt && "저장됨 · " + savedAt}</span>
        </div>
      </div>
    </div>
  );
