  ...
  return (
    <section className="mt-5 overflow-hidden rounded-card border border-line bg-panel shadow-card">
      <div className="flex flex-wrap items-center gap-3 border-b border-line px-5 py-4">
        <h2 className="m-0 flex items-center gap-[9px] text-base tracking-[-0.2px]">📓 데일리 업무일지</h2>
        <div className="flex items-center gap-1.5 text-[12.5px] text-ink-2">
          <button type="button" className="btn btn-tiny btn-ghost" title="하루 전" onClick={() => onShift(-1)}>
            ‹
          </button>
          <input type="date" value={curDate} onChange={(e) => e.target.value && onPickDate(e.target.value)} />
          <button type="button" className="btn btn-tiny btn-ghost" title="하루 뒤" onClick={() => onShift(1)}>
            ›
          </button>
        </div>
