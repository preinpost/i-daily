...
import { emptyDoc, normCfg, normalizeDoc, shiftDate, ymd } from "./lib/model";
...
export function App() {
  ...
  const [meta, setMeta] = useState<Meta>({ today: null, owner: "", jiraBase: "" });
  ...
  const setDot = (cls: string, note?: string) => setSaveState((s) => ({ cls, note: note ?? s.note }));
  ...
  async function loadDate(date: string) {
    if (dirty.current && date !== curDateRef.current && !confirm("저장하지 않은 변경이 있습니다. 저장하지 않고 이동할까요?")) {
      return;
    }
    setCurDate(date);
  ...
  async function saveNow(): Promise<boolean> {
    ...
    if (r.ok && r.json) {
      setDirty(false);
      applyTeams(r.json.teams, r.json.teamsHtml || "");
      setDot("", "저장됨 · " + new Date().toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" }));
      toast("저장됨");
      return true;
    }
  ...
  async function revert() {
    ...
    if (!confirm("저장하지 않은 변경을 모두 버리고 마지막 저장 상태로 되돌릴까요?")) return;
    setDot("busy", "되돌리는 중…");
  ...
  async function carry() {
    if (dirty.current && !confirm("저장하지 않은 변경이 있습니다. 이월하면 사라집니다. 계속할까요?")) return;
    if (
      !confirm(
      ...
      return;
    const r = await api<any>("POST", "/api/day/" + curDateRef.current + "/carry");
    if (r.ok && r.json) {
      docRef.current = r.json.data;
      normalizeDoc(docRef.current!);
      applyTeams(r.json.teams, r.json.teamsHtml || "");
    ...
    try {
      if (teamsHtmlRef.current && window.ClipboardItem) {
        await navigator.clipboard.write([
          new ClipboardItem({
            "text/html": new Blob([teamsHtmlRef.current], { type: "text/html" }),
  ...
  function onConfigSaved(cfg: Config, _configured: boolean) {
    ...
    metaRef.current = { ...metaRef.current, owner: cfg.owner, jiraBase: cfg.jiraBase };
  }
  ...
  if (!ready || !docRef.current) {
    return (
      <>
        {banner}
        <div className="mx-auto max-w-[1080px] px-[18px] py-10 text-ink-2">불러오는 중…</div>
      </>
    );
  }

  return (
    <EditorContext.Provider value={{ doc: docRef.current, meta, config, commit, rerender: bump }}>
      {banner}
      ...
      <WeeklyReportPane active={view === "report"} config={config} onSaved={onConfigSaved} />
      <ConfigPane active={view === "config"} config={config} firstRun={firstRun} onSaved={onConfigSaved} />
