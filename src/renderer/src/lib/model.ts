...
import type { Block, Config, Doc, ListItem, Meta, Scrum, Section, Space, Task, Ticket, Which } from "../types";

type CopySrc = { key?: string; desc?: string; progress?: number | ""; due?: string; subs?: string[] };
...
export function guessSpace(config: Config, key: string): string {
  const p = (key || "").split("-")[0].toUpperCase();
  if (!p) return "";
  const r = (config.spaceRules || []).find((x) => (x.prefix || "").toUpperCase() === p);
  return r ? r.space : "";
}
...
export function listHasContent(sec: { items?: ListItem[] }): boolean {
  return (sec.items || []).some(
    (it) => (it.key || "").trim() || (it.desc || "").trim() || (it.subs || []).some((s) => (s || "").trim()),
  );
}
...
export function blockHasContent(b?: Block): boolean {
  if (!b) return false;
  if ((b.issues || "").trim() && (b.issues || "").trim() !== "없음") return true;
  if ((b.collab || "").trim() && (b.collab || "").trim() !== "없음") return true;
  return (b.spaces || []).some(
    (sp) =>
      (sp.label || "").trim() ||
      (sp.tasks || []).some(
        (t) => (t.key || "").trim() || (t.desc || "").trim() || (t.subs || []).some((s) => (s || "").trim()),
      ),
  );
}
...
export function normalizeDoc(d: Doc): void {
  ...
  (["prev", "today"] as Which[]).forEach((w) => {
    ...
    (d.scrum[w].spaces || []).forEach((sp) => (sp.tasks || []).forEach((t) => { if (!t.subs) t.subs = []; }));
  });
  if (!d.sections || !d.sections.length) d.sections = emptyDoc({ today: null, owner: d.owner, jiraBase: "" }, d.date).sections;
...
/* ── 일일 진행(마스터) ↔ 스크럼 연결 ── */
export function listSection(doc: Doc): (Section & { kind: "list" }) | undefined {
  return doc.sections.find((x) => x.kind === "list") as (Section & { kind: "list" }) | undefined;
}
...
export function dailyItemLabel(it: { key?: string; desc?: string }, order?: number | null): string {
  const k = (it.key || "").trim();
...
export function applyDailyMaster(t: Task, src: { key?: string; desc?: string; subs?: string[] }): void {
  if (!t || !src) return;
  t.key = src.key || "";
  t.desc = src.desc || "";
...
export function syncScrumFromDailyChange(scrum: Scrum, prevId: string, it: ListItem): void {
  const newId = itemId(it);
  (["prev", "today"] as Which[]).forEach((which) => {
    (scrum[which].spaces || []).forEach((sp) =>
      (sp.tasks || []).forEach((t) => {
        const tid = itemId(t);
        if ((prevId && tid === prevId) || (newId && tid === newId)) applyDailyMaster(t, it);
      }),
    );
  });
}
...
export function copyListItemToScrum(
  ...
  if (isScrumDup(block, it as ListItem)) return { ok: false, msg: "이미 " + (which === "prev" ? "전일" : "금일") + "에 있어요" };
  let sp = targetSp || null;
  if (!sp) {
    const label = guessSpace(config, it.key || "");
    sp = (block.spaces || []).find((s) => (s.label || "") === label) || null;
    if (!sp) {
      sp = { label, tasks: [] };
  ...
  return { ok: true, msg: (it.key || it.desc) + " → " + (which === "prev" ? "전일" : "금일") + " 등록" };
}

export function isMissing(t: Task): boolean {
  return (t.progress === "" || t.progress == null || t.due === "") && !!(t.key || t.desc);
}

/* ── 티켓 → 일일 항목 보장 ── */
export function ticketToItem(t: { key?: string; summary?: string; due?: string }): ListItem {
  return {
    done: false,
...
export function normCfg(c?: Partial<Config> | null): Config {
  ...
  return {
    ...
    spaceRules: Array.isArray(s.spaceRules) ? s.spaceRules : [],
...
export function rulesToText(rules: { prefix: string; space: string }[]): string {
  return (rules || []).map((r) => (r.prefix || "") + "=" + (r.space || "")).join("\n");
}
export function textToRules(text: string): { prefix: string; space: string }[] {
  return (text || "")
    .split("\n")
    .map((line) => {
      const i = line.indexOf("=");
      if (i < 0) return null;
      const prefix = line.slice(0, i).trim().toUpperCase();
      const space = line.slice(i + 1).trim();
      return prefix ? { prefix, space } : null;
    })
    .filter(Boolean) as { prefix: string; space: string }[];
}
