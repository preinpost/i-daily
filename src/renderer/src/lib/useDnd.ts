...
export function useReorder(arr: (Task | ListItem)[], index: number, onDrop: () => void) {
...
export function useSubReorder(arr: string[], index: number, onDrop: () => void) {
  const [over, setOver] = useState(false);
  return {
    over,
    handleProps: {
...
// 일일 진행 → 스크럼 복사 드롭존.
export function useCopyZone(onCopy: (item: Task) => void) {
  const [over, setOver] = useState(false);
  return {
    over,
    props: {
      ...
      onDrop: (e: DragEvent) => {
        if (drag.kind !== "list" || !drag.item) return;
        e.preventDefault();
        e.stopPropagation();
        setOver(false);
        onCopy(drag.item);
