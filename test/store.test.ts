...
import { sqliteStore, queryTasks, createDb } from "../src/shared/store.ts";
...
test("sqliteStore 왕복 + 파생 tasks 쿼리 + 유저 격리", async () => {
  ...
  expect((await store.get("2026-07-10"))!.scrum.today.spaces[0].tasks[0].key).toBe("OPIT-1");
...
test("shortcuts 유저별 격리 + 순서 보존", async () => {
  const db = freshDb();
  await sqliteStore(db, "u1").putShortcuts([{ name: "Jira", url: "https://x" }, { name: "Wiki", url: "https://y" }]);
  expect(await sqliteStore(db, "u1").getShortcuts()).toEqual([{ name: "Jira", url: "https://x" }, { name: "Wiki", url: "https://y" }]);
  expect(await sqliteStore(db, "u2").getShortcuts()).toEqual([]);
  await sqliteStore(db, "u1").putShortcuts([{ name: "Only", url: "https://z" }]);   // 교체 시 잔여 없음
  expect(await sqliteStore(db, "u1").getShortcuts()).toEqual([{ name: "Only", url: "https://z" }]);
});
