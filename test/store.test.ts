...
test("일일 진행 업무 항목의 space 필드 왕복(DB 오가서 푸리)", async () => {
	const store = d1Store(await freshDb(), "u1");
	const md = [
		"## 일일 진행 업무",
		"- 무그룹 건",
		"  + **[backend]**",
		"    + [OPIT-3](u) 백엔드 작업 (10%, ~7/20)",
		"",
		"## 데일리 스크럼",
		"",
		"**[금일 진행 업무]**",
	...
	const got = (await store.get("2026-07-10"))!;
	const list: any = got.sections.find((s) => s.kind === "list");
	expect(list.items.map((it: any) => it.space)).toEqual(["", "backend"]);
	expect(list.items[1].key).toBe("OPIT-3");
	expect(list.items[1].progress).toBe(10);
});
...
test("shortcuts 유저별 격리 + 순서 보존", async () => {
	...
	await d1Store(db, "u1").putShortcuts([{ name: "Only", url: "https://z" }]); // 교체 시 잔여 없음
	expect(await d1Store(db, "u1").getShortcuts()).toEqual([
