...
test("route: config 최초 firstRun → PUT 저장 → configured", async () => {
  ...
  const put = await route("PUT", "/api/config", {
    owner: "홍길동", jiraBase: "https://jira.test",   // host만 — /browse/ 자동
    spaces: ["backend"], spaceRules: [{ prefix: "ABC", space: "backend" }],
  }, U, d);
...
test("route: PUT day 저장 후 /api/tasks 쿼리(대소문자 무관)", async () => {
  ...
  expect(put.status).toBe(200);
  const q = await route("GET", "/api/tasks?key=opit&side=today", undefined, U, d);
...
test("route: prev-daily가 직전 일일 items + block 반환", async () => {
  ...
  const r = await route("GET", "/api/day/2026-07-11/prev-daily", undefined, U, d);
  expect(r.status).toBe(200);
